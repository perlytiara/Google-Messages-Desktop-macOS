...
const VERIFICATION_FORMATS = [
  ...
  {
    label: 'French — BoursoBank le code est',
    payload: {
      title: 'BoursoBank',
      body: 'BoursoBank : pour "Déblocage accès web client" le code est 239159. Soyez vigilant, un conseiller ne vous appellera jamais pour vous demander ce code par téléphone',
    },
  },
  {
...
const SPACED_TEST_SCENARIO = {
  label: 'Spaced test sequence (10s apart)',
  payloads: [
    {
      ...REGULAR_SCENARIO.payload,
      body: 'Test 1 of 2 — regular message with reply and Mark Read.',
    },
    {
      title: 'BoursoBank',
      body: 'BoursoBank : pour "Déblocage accès web client" le code est 239159. Soyez vigilant, un conseiller ne vous appellera jamais pour vous demander ce code par téléphone',
      ...
      raw: JSON.stringify({
        source: 'test',
        sender: 'BoursoBank',
        body: 'BoursoBank : pour "Déblocage accès web client" le code est 239159.',
        conversationUrl: TEST_CONVERSATION_URL,
      }),
    },
  ],
...
const BATCH_TEST_SCENARIO = {
  label: 'Batch notification test (5 messages, 8s apart)',
  intervalMs: 8 * 1000,
  payloads: [
    REGULAR_SCENARIO.payload,
    GROUPED_SCENARIO.payload,
    {
      title: 'BoursoBank',
      body: 'BoursoBank : pour "Déblocage accès web client" le code est 239159. Soyez vigilant, un conseiller ne vous appellera jamais pour vous demander ce code par téléphone',
      data: TEST_CONVERSATION_URL,
    ...
    VERIFICATION_FORMATS.find((item) => item.label.includes('G-482913')).payload,
    VERIFICATION_FORMATS.find((item) => item.label.includes('Tinder')).payload,
  ],
};
...
function runSpacedNotificationTest({ payloads, intervalMs = 10 * 1000 }) {
  payloads.forEach((payload, index) => {
    setTimeout(() => {
      simulateNotification(payload);
      console.log('[Messages] Test notification', index + 1, 'of', payloads.length, ':', payload.title || payload.body?.slice(0, 40));
    }, index * intervalMs);
  });
}

function runBatchNotificationTest(scenario = BATCH_TEST_SCENARIO) {
  console.log('[Messages] Starting batch notification test:', scenario.label);
  runSpacedNotificationTest(scenario);
}
