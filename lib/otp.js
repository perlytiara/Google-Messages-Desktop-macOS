const PRIORITY_PATTERNS = [
  ...
  /(?:verification\s+message|verification\s+code|\bcode\b|\botp\b|\bpasscode\b|\bpin\b)\D{0,32}(\d{4,8})/i,
  /G[\-\u2010\u2011\u2012\u2013\u2014\u2212](\d{6})/i,
...
function extractOtp(text) {
  ...
  if (/verif|otp|\bcode\b|\bG-/i.test(normalized)) {
    const loose = /\b(\d{6})\b/.exec(normalized) || /\b(\d{4,8})\b/.exec(normalized);
    if (loose?.[1]) {
      return loose[1];
    }
  }
...
function extractAllOtps(text) {
  ...
  for (const match of normalized.matchAll(/\b([A-Z]\d{6})\b/g)) {
    const code = match[1];
    if (!seen.has(code)) {
      seen.add(code);
      results.push(code);
