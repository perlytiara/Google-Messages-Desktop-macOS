...
function readSelfTestConfig() {
  try {
    ...
    // Strict opt-in only — legacy "enabled" must not turn this on.
    const notifyOutgoing = parsed.notifyOutgoing === true;
...
function writeSelfTestConfig(config) {
  const next = {
    notifyOutgoing: config.notifyOutgoing === true,
