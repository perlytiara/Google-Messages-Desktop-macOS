...
const DEFAULT_PUBLISH = {
  provider: 'github',
  owner: 'perlytiara',
  repo: 'Google-Messages-Desktop-macOS',
  releaseType: 'release',
};
...
function readPublishConfig() {
  try {
    ...
    if (parsed?.build?.publish?.provider === 'github') {
      return parsed.build.publish;
    }
  } catch {
...
function configureUpdateFeed() {
  ...
  console.log('[Messages] Update feed:', `${publish.owner}/${publish.repo}`);
}
...
function closeUpdateWindowForQuit() {
  if (updateWindow && !updateWindow.isDestroyed()) {
